...
const TechnicalSkills = () => {
    ...
    const buttonStyling =
        'bg-[#1e1e1e] rounded-xl shadow-lg p-4 border hover:border-white hover:scale-105  border-[#1e1e1e] hover:cursor-pointer transition-transform duration-200';
    ...
    return (
        <div className="m-5">
            ...
            <p className="text-sm text-zinc-300">
                {' '}
                *click to see the languages, framesworks, projects made with the
                language
            </p>
            ...
            {modalIndex !== null && (
                <LanguageModal
                    ...
                    onPrev={() =>
                        setModalIndex(
                            index =>
                                (index - 1 + languageInfo.length) %
                                languageInfo.length
                        )
                    }
                    onNext={() =>
                        setModalIndex(
                            index => (index + 1) % languageInfo.length
                        )
                    }
                />
            )}
